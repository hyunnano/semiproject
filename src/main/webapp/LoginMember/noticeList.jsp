...
List<NoticeDTO> noticeLists = dao.selectListPage(param); //페이징 게시물
dao.close(); // DB 연결 닫기
%>
...
<!-- 검색부분 -->
<h2 style="text-align:center;">공지사항 게시글</h2>
...
<table border="1" width="1500px" style="margin: 5px auto; ">
	...
	<tr align="center">
		...
		<!--게시물 번호-->
		<td align="left">
			<!--제목(+ 하이퍼링크)--> <a href="noticeView.jsp?n_num=<%= dto.getN_num() %>"><%= dto.getN_title() %>
		</a>
		</td>
		<td align="center"><%=dto.getId()%></td>
		<!--작성자 아이디-->
		<td align="center"><%=dto.getN_visitcount()%></td>
		<!--조회수-->
		<td align="center"><%=dto.getN_date()%></td>
		<!--작성일-->
	</tr>
	<%
}
...
<span style = "display : flex; justify-content :center;"><%= BoardPage.pagingStr(totalCount, pageSize, blockPage, pageNum, request.getRequestURI()) %></span>
    ...
    <span style="float:right;"><button type="button" onclick="location.href='noticeWrite.jsp';"
